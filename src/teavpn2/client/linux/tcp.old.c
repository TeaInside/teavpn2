...
static int do_handshake(struct cli_thread *thread)
{
	...
	prl_notice(0, "Performing handshake...");
...
again:
	...
	if (unlikely(tmp_ret < 0)) {
		pr_err("send_to_server(): " PRERF, PREAR((int)-tmp_ret));
		return tmp_ret;
	}
	...
	if (unlikely((size_t)send_ret > send_len)) {
		panic("Bug: (size_t)send_ret > send_len (send_ret=%zd; "
		      "send_len=%zu)", send_ret, send_len);
		abort();
	}
...
static int do_handshake_event_loop(struct cli_thread *thread,
				   struct epoll_event events[EPL_WAIT_NUM])
{
	uint8_t i = 0;
	int ret = -ENETDOWN;
...
static int run_main_thread(struct cli_thread *thread)
{
	...
	do_handshake_event_loop(thread, events);
...
static int run_workers(struct cli_state *state)
{
	...
	for (i = 0; i < nn; i++) {
		...
		ret = epoll_add(thread->epoll_fd, tun_fds[i - 1], EPL_IN_EVT);
		if (unlikely(ret))
			goto out_err;
