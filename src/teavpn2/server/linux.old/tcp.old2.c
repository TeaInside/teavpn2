			 ...
			 struct epoll_event events[EPL_WAIT_ARRSIZ])
{
	...
	pr_debug("epoll_wait() (thread=%u) (epoll_fd=%d)", thread->idx,
		 thread->epoll_fd);
			    ...
			    struct srv_thread *thread)
{
	...
	ret = register_client(cli_fd, &saddr, thread);
out:
	return ret;
}
			      ...
			      struct srv_state *state)
{
	...
	pr_debug("epoll_delete(%d, %d) (thread=%u)", thread->epoll_fd, cli_fd, thread->idx);
	close(cli_fd);
	pr_debug("state->epoll_map[%d] = NOP (thread=%u)", cli_fd, thread->idx);
...
out:
	// if (thread->idx)
	// 	usleep(1000u * ((unsigned)rand() % 1000u));
out_close:
