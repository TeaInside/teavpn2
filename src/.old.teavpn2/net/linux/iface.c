...
int tun_alloc(const char *dev, int flags)
{
	...
	if (unlikely(fd < 0)) {
		err = errno;
		pr_error("open(\"/dev/net/tun\", O_RDWR): " PRERR, PREAG(err));
		return -err;
	}
	...
	if (unlikely(ioctl(fd, TUNSETIFF, &ifr) < 0)) {
		err = errno;
		pr_error("ioctl(%d, TUNSETIFF, &ifr): " PRERR, fd, PREAG(err));
		return -err;
	}
...
int fd_set_nonblock(int fd)
{
	...
	if (unlikely(flags < 0)) {
		err = errno;
		pr_error("fcntl(%d, F_GETFL, 0): " PRERR, fd, PREAG(err));
		return -err;
	}
	...
	if (unlikely(flags < 0)) {
		err = errno;
		pr_error("fcntl(%d, F_SETFL, %d): " PRERR, fd, flags,
			 PREAG(err));
		return -err;
	}
	...
	if (ioctl(fd, FIONBIO, &flags) < 0) {
		err = errno;
		pr_error("ioctl(%d, FIONBIO, &flags): " PRERR, fd, PREAG(err));
		return -err;
	}
...
bool iface_up(struct iface_rd_cfg *iface)
{
